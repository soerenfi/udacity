...
class FlatDrawable : public SceneGraph::Drawable3D {
   public:
    explicit FlatDrawable(
        Object3D& object, Shaders::FlatGL3D& shader, GL::Mesh& mesh, SceneGraph::DrawableGroup3D& drawables)
        : SceneGraph::Drawable3D{object, &drawables}, _shader(shader), _mesh(mesh) {}
...
MouseInteractionExample::MouseInteractionExample(const Arguments& arguments)
       ...
       MSAA if we have enough DPI. */
    {
        ...
        Configuration conf;
        conf.setTitle("Magnum Mouse Interaction Example").setSize(conf.size(), dpiScaling);
    ...
    _mesh.setCount(3).addVertexBuffer(
        std::move(buffer), 0, Shaders::VertexColorGL3D::Position{}, Shaders::VertexColorGL3D::Color3{});
...
void MouseInteractionExample::keyPressEvent(KeyEvent& event) {
    ...
    if (event.key() == KeyEvent::Key::NumZero) {
        ...
        event.key() == KeyEvent::Key::NumOne || event.key() == KeyEvent::Key::NumThree ||
...
void MouseInteractionExample::mousePressEvent(MouseEvent& event) {
    /* Due to compatibility reasons, scroll is also reported as a press event,
       ...
       gone from Magnum. */
    if (event.button() != MouseEvent::Button::Left && event.button() != MouseEvent::Button::Middle) return;
