...
void Render::DrawVehicle(io2d::output_surface& surface) const {
    io2d::render_props aliased{io2d::antialias::none};
    ...
    pb.new_figure({(float)(std::rand() % 100) / 100, (float)(std::rand() % 100) / 100});
    float constexpr l_marker = 0.01f;
    pb.rel_line({l_marker, 0.f});
    pb.rel_line({0.f, l_marker});
    pb.rel_line({-l_marker, 0.f});
...
void Render::DrawWater(io2d::output_surface& surface) const {
    for (auto& water : m_Model.Waters()) surface.fill(m_WaterFillBrush, PathFromMP(water));
}
...
void Render::DrawRailways(io2d::output_surface& surface) const {
    ...
    for (auto& railway : m_Model.Railways()) {
        ...
        surface.stroke(
            m_RailwayStrokeBrush, path, std::nullopt, io2d::stroke_props{m_RailwayOuterWidth * m_PixelsInMeter});
        surface.stroke(
            m_RailwayDashBrush, path, std::nullopt, io2d::stroke_props{m_RailwayInnerWidth * m_PixelsInMeter},
            m_RailwayDashes);
    }
}
...
io2d::interpreted_path Render::PathLine() const {
    ...
    for (int i = 1; i < m_Model.path.size(); i++) pb.line(ToPoint2D(m_Model.path[i]));
...
io2d::interpreted_path Render::PathFromWay(const Model::Way& way) const {
    ...
    for (auto it = ++way.nodes.begin(); it != std::end(way.nodes); ++it) pb.line(ToPoint2D(nodes[*it]));
    return io2d::interpreted_path{pb};
}
